...
/* ===== Account page: de-dupe + pretty summary + counts ===== */
(function fixAccountPage(){
  ...
  function dedupeCards(){
    const page = byId('page-account'); if (!page) return;
    const cards = [...page.querySelectorAll('.account-card')];
    let sawAcc=false, sawSubs=false, sawAdmin=false;

    cards.forEach(card=>{
      const title = norm(card.querySelector('h3')?.textContent);
      if (title==='profile') { card.remove(); return; }          // kill stray “Profile” card
      if (title==='my account') { if (sawAcc) card.remove(); else sawAcc=true; return; }
      if (title==='my submissions') { if (sawSubs) card.remove(); else sawSubs=true; return; }
      if (title==='admin tools') { if (sawAdmin) card.remove(); else sawAdmin=true; return; }
    });
  }

  function ensurePrettyAccount(){
    const wrap = $('#page-account .account-wrap') || byId('page-account');
    if (!wrap) return;
    if (byId('acctName')) return; // already present
  ...
  function paintSummary(){
    const user = getSession();
    const name = user?.display || deriveName(user?.email);
    byId('acctName')      && (byId('acctName').textContent = name);
    byId('acctEmailLine') && (byId('acctEmailLine').textContent = user?.email || '');
    byId('acctRoleLine')  && (byId('acctRoleLine').textContent = user?.role ? ('Role: ' + user.role) : '');
  ...
  function renderMySubsChips(){
    const subsCard = [...document.querySelectorAll('#page-account .account-card')]
      .find(c => norm(c.querySelector('h3')?.textContent) === 'my submissions');
    if (!subsCard) return;
    if (subsCard.querySelector('.mychips')) return; // already added

    const counts = (typeof allApproved==='function')
      ? allApproved().reduce((m,g)=>{ const t=(g.type||'song'); m.all++; m[t]=(m[t]||0)+1; return m; }, {all:0})
      : {all:0,song:0,pattern:0};

    subsCard.insertAdjacentHTML('afterbegin', `
      <div class="mychips" style="display:flex;gap:8px;margin:0 0 8px">
        <span class="chip">All (${counts.all||0})</span>
        <span class="chip">Songs (${counts.song||0})</span>
        <span class="chip">Patterns (${counts.pattern||0})</span>
      </div>
    `);
  }
